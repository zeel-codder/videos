...
def get_latest_videos(query="football", type="video", maxResults=50, order="date"):
    ...
    Args:
        ...
        maxResults (int, optional): _Total size of item per page_. Defaults to 50.
        order (int, optional): _Sort data as per order value_. Defaults to 50.
    Returns:
    ...
    try:
        ...
        if e.status_code == 403:
            ...
            yt_api_key_data = KeyModel.objects.get(type="YT")
            yt_api_key_data.index = (yt_api_key_data.index + 1) % size
            yt_api_key_data.save()
        return False
